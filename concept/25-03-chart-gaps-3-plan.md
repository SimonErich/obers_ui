...
- [ ] `lib/src/composites/oi_polar_chart.dart` — Mix in `ChartStreamingHost`. Override `streamingSeries`, `syncGroup`. Wire initState/dispose.
- [ ] `lib/src/composites/oi_matrix_chart.dart` — Same wiring
- [ ] `lib/src/composites/oi_hierarchical_chart.dart` — Same (no normalization, no streaming needed for tree data)
- [ ] `lib/src/composites/oi_flow_chart.dart` — Same
- [ ] TDD: Cartesian chart with streaming source → adapter emits data → chart rebuilds with new data
- [ ] TDD: Cartesian chart with syncGroup → registers on mount, unregisters on dispose
- [ ] TDD: Cartesian chart with settings → controller state restored from persisted settings on mount
...
- [ ] `lib/src/composites/oi_line_chart/oi_line_chart.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_candlestick_chart/oi_candlestick_chart.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_bubble_chart/oi_bubble_chart.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_bubble_chart/oi_bubble_chart_legend.dart` — Replace 1 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_bubble_chart/oi_bubble_chart_size_legend.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_scatter_plot.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_radar_chart.dart` — Replace 1 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_heatmap.dart` — Replace 3 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_treemap.dart` — Replace 2 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_gauge.dart` — Replace 1 `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_chart_series_toggle.dart` — Replace 1 `Text()` → `OiLabel`
- [ ] `lib/src/models/oi_series_style.dart` — Add `OiSeriesStyle.dashed({Color? strokeColor, double? strokeWidth})` and `OiSeriesStyle.dotted({Color? strokeColor, double? strokeWidth})` factory constructors
- [ ] Verify: `dart analyze` && `flutter test`

### Phase 3: Integration Tests

- **Goal**: Prove streaming, sync, persistence, and normalization wiring works
- [ ] `test/src/composites/oi_cartesian_chart_wiring_test.dart`:
...
- [ ] `test/src/composites/oi_cartesian_chart_sync_test.dart`:
