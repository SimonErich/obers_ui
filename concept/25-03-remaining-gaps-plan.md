...
- [ ] `lib/src/models/oi_chart_settings.dart` — Add `OiPersistedViewport` class (xMin, xMax, yMin, yMax doubles, toJson/fromJson, ==, hashCode). Add `OiPersistedSelection` class (selectedRefs as List of {seriesIndex, dataIndex} maps, toJson/fromJson). Replace `viewportWindow: Rect?` → `viewport: OiPersistedViewport?`. Replace `selectedSeriesIndex: int?` + `selectedDataIndex: int?` → `selection: OiPersistedSelection?`. Change `legendExpandedGroups: Set<String>` → `Map<String, bool>`. Bump schema version. Update toJson/fromJson/mergeWith/copyWith/==/hashCode.
- [ ] `lib/src/models/oi_chart_complexity.dart` — Create `OiChartComplexity` enum: `mini`, `standard`, `detailed`
- [ ] `lib/src/composites/oi_cartesian_chart.dart` — Add `complexity: OiResponsive<OiChartComplexity>?` param + field
- [ ] Export `oi_chart_complexity.dart` from barrel
- [ ] TDD: OiPersistedViewport round-trips via toJson/fromJson
- [ ] TDD: OiPersistedSelection round-trips refs correctly
- [ ] TDD: OiChartSettings with new types serializes/deserializes without loss
...
- [ ] `lib/src/foundation/oi_chart_tooltip.dart` — Replace `Text()` → `OiLabel.caption()` / `OiLabel.body()`. Replace `Column` → `OiColumn`, `Row` → `OiRow`. Replace 6 hardcoded `Color(0x...)` → theme tokens from `OiChartTooltipTheme` or `context.colors`
- [ ] `lib/src/components/oi_chart_tooltip_widget.dart` — Same: `Text()` → `OiLabel`, hardcoded colors → theme
- [ ] `lib/src/composites/oi_chart_legend.dart` — Replace `Row` → `OiRow`, `Text` → `OiLabel.body()`
- [ ] `lib/src/composites/oi_pie_chart.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_funnel_chart.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_sankey.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_bar_chart/oi_bar_chart.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_bar_chart/oi_bar_chart_legend.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_combo_chart.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_area_chart/oi_area_chart.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/composites/oi_area_chart/oi_area_chart_legend.dart` — Replace `Text()` → `OiLabel`
- [ ] `lib/src/components/oi_chart_crosshair_widget.dart` — Replace fallback `Color(0x80888888)` → theme
- [ ] `lib/src/components/oi_chart_axis_widget.dart` — Replace 3 fallback colors → theme tokens
- [ ] `lib/src/components/oi_chart_annotation_layer.dart` — Replace fallback color → theme
- [ ] Verify: `dart analyze` && `flutter test`

### Phase 3: Composite Wiring — Sync + Persistence + Normalization

- **Goal**: Wire remaining orchestration into composites
- [ ] `lib/src/composites/_chart_behavior_host.dart` — Add `syncGroup` getter override requirement. In `attachBehaviors()`, if syncGroup set, look up `OiChartSyncProvider.maybeOf(context)` → call `addViewportListener`/`addCrosshairListener`. In `disposeBehaviorHost()`, remove listeners. Add `persistSettings()` method: if settings binding provided, serialize controller state to `OiChartSettings` and save.
- [ ] `lib/src/composites/oi_cartesian_chart.dart` — Override `syncGroup` getter. In build, after computing visible series, call `normalizeSeries()` on each to produce `List<OiChartDatum>`. Apply decimation if `performance?.decimation != OiChartDecimationStrategy.none`. Store normalized datums for optional use by seriesBuilder.
- [ ] `lib/src/composites/oi_polar_chart.dart` — Override `syncGroup`. Wire streaming host `attachStreamingAdapters()` in initState.
- [ ] `lib/src/composites/oi_matrix_chart.dart` — Same wiring
- [ ] `lib/src/composites/oi_hierarchical_chart.dart` — Same (no normalization)
- [ ] `lib/src/composites/oi_flow_chart.dart` — Same (no normalization)
- [ ] TDD: Cartesian chart registers with sync coordinator on mount when syncGroup is set
- [ ] TDD: Cartesian chart unregisters from sync coordinator on dispose
- [ ] TDD: Cartesian chart normalizes series data into OiChartDatum list
- [ ] Verify: `dart analyze` && `flutter test`

### Phase 4: Test Coverage — Legend Widget

- **Goal**: Add comprehensive legend widget tests
- [ ] `test/src/composites/oi_chart_legend_test.dart` — Tests:
...
- [ ] `test/src/behaviors/oi_chart_tooltip_behavior_test.dart` — Tests:
...
- [ ] `test/src/behaviors/oi_chart_crosshair_behavior_test.dart` — Tests:
...
- [ ] `test/src/behaviors/oi_chart_brush_behavior_test.dart` — Tests:
...
- [ ] `test/src/composites/oi_cartesian_chart_wiring_test.dart` — Tests:
