...
class OiHitTestConfig {
  ...
  @override
  int get hashCode => Object.hash(tolerance, preference, bucketWidth, includeRadius);
}
...
class OiChartHitTestIndex implements OiChartHitTester {
  ...
  OiChartHitTestIndex({
    this.config = const OiHitTestConfig(),
  });
  ...
  List<OiChartHitResult> _findNearby(Offset position, double tolerance) {
    ...
    final startBucket = (centerBucket - bucketSpan).clamp(0, _buckets.length - 1);
