...
class _OiButtonState extends State<OiButton> {
  ...
  // ── Hover / pressed state for variant style colors ────────────────────────
  bool _hovered = false;
  bool _pressed = false;
  ...
  Color _backgroundColor(
    ...
    // Return state-specific colors from variant style when available.
    if (pressed && vs?.backgroundPressed != null) return vs!.backgroundPressed!;
    if (hovered && vs?.backgroundHover != null) return vs!.backgroundHover!;
  ...
  Color _foregroundColor(
    ...
    final vs = _variantStyle(context.components.button, variant);
    if (pressed && vs?.foregroundPressed != null) return vs!.foregroundPressed!;
    if (hovered && vs?.foregroundHover != null) return vs!.foregroundHover!;
  ...
  BoxDecoration _decoration(
    ...
    // Resolve border: check variant style override first (state-aware), then default
    final vs = _variantStyle(bt, variant);
    final Border? border;
    if (pressed && vs?.borderPressed != null) {
      border = Border.all(color: vs!.borderPressed!);
    } else if (hovered && vs?.borderHover != null) {
      border = Border.all(color: vs!.borderHover!);
  ...
  /// Whether the variant style has explicit hover or pressed colors configured.
  bool _hasVariantStateColors(BuildContext context, OiButtonVariant variant) {
    final vs = _variantStyle(context.components.button, variant);
    if (vs == null) return false;
    return vs.backgroundHover != null ||
        vs.backgroundPressed != null ||
        vs.foregroundHover != null ||
        vs.foregroundPressed != null ||
        vs.borderHover != null ||
        vs.borderPressed != null;
  }
  ...
  Widget _buildStandardButton(BuildContext context) {
    ...
    final hasStateColors = _hasVariantStateColors(context, widget.variant);
    final foreground = _foregroundColor(context, widget.variant,
        hovered: _hovered, pressed: _pressed);
    final themeRadius = bt?.borderRadius;
    ...
    // When variant style has explicit hover/pressed colors, we manage state
    // ourselves and wrap with MouseRegion + GestureDetector so OiTappable's
    // generic overlay doesn't fight with the explicit colors.
    if (hasStateColors) {
      inner = MouseRegion(
        cursor:
    ...
    Widget button;
    if (hasStateColors) {
      // Add semantics since we're bypassing OiTappable.
      final semLabel = widget.semanticLabel ?? widget.label;
      button = semLabel != null
          ? Semantics(
              label: semLabel,
              button: true,
  ...
  Widget _buildGhostButton(BuildContext context) {
    ...
    // Resolve background for ghost: transparent by default, but honour
    // variant style hover/pressed colors when configured.
    final ghostBg = _highlighted ? vs?.backgroundHover : null;

    Widget content = Container(
      height: height,
      padding: EdgeInsets.symmetric(horizontal: hPad),
      decoration: ghostBg != null
          ? BoxDecoration(color: ghostBg, borderRadius: effectiveRadius)
          : null,
      child: Center(
        widthFactor: 1.0,
        child: _buildContent(
