  ...
  ({int seriesIndex, int pointIndex})? _findNearest(
    ...
    for (var si = 0; si < widget.series.length; si++) {
      ...
      for (var pi = 0; pi < series.points.length; pi++) {
        ...
        final px = chartRect.left +
            chartRect.width * range.normaliseX(point.x);
        final py = chartRect.bottom -
            chartRect.height * range.normaliseY(point.y);
    ...
    return Semantics(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final chartSize = Size(w, math.min(h, isCompact ? w * 0.6 : w * 0.75));
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var si = 0; si < series.length; si++) {
      ...
      for (var pi = 0; pi < s.points.length; pi++) {
        ...
        final px =
            chartRect.left + chartRect.width * range.normaliseX(point.x);
        final py = chartRect.bottom -
            chartRect.height * range.normaliseY(point.y);
        final center = Offset(px, py);

        final isHovered =
            hoveredSeriesIndex == si && hoveredPointIndex == pi;
