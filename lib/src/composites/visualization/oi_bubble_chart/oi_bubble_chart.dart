...
import 'package:obers_ui/src/composites/visualization/oi_bubble_chart/oi_bubble_chart_legend.dart';
...
class _OiBubbleChartState extends State<OiBubbleChart> {
  ...
  List<ResolvedBubble> _resolveBubbles(BuildContext context) {
    ...
    if (data.series.isEmpty ||
        data.series.every((s) => s.points.isEmpty)) {
      return resolved;
    }
    ...
    final sizeConfig = data.sizeConfig ??
    ...
    for (var si = 0; si < data.series.length; si++) {
      ...
      for (var pi = 0; pi < series.points.length; pi++) {
        ...
        final radius = sizeConfig.minRadius +
  ...
  void _handleTapDown(TapDownDetails details, List<ResolvedBubble> bubbles, Size size, bool isCompact) {
  ...
  void _handleHover(PointerEvent event, List<ResolvedBubble> bubbles, Size size, bool isCompact) {
    final rect = _chartRect(size, isCompact);
  ...
  Widget _buildChartBody(
    ...
    if (activeIdx != null && activeIdx < bubbles.length) {
      ...
      final desc =
          OiBubbleChartAccessibility.describeBubble(point, series.name);
      narration = Semantics(
    ...
    return Semantics(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final isCompact =
              widget.compact ?? (w < _compactThreshold);
