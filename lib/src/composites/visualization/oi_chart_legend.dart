...
///
/// {@category Composites}
typedef OiLegendItemBuilder = Widget Function(
...
class OiChartLegend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final labelStyle = (theme?.labelStyle ??
  ...
  Widget _buildItem(
    ...
    final onDoubleTap =
        onExclusiveFocus != null ? () => onExclusiveFocus!(item.id) : null;
...
class _LegendMarker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      size: Size(
        shape == OiLegendMarkerShape.line ? size * 1.5 : size,
        size,
      ),
