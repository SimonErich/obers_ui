...
void main() {
  ...
  testWidgets('renders Icon widget', (tester) async {
    await tester.pumpObers(
      const OiIcon(icon: testIcon, label: 'test icon'),
    );
  ...
  testWidgets('semantic icon has Semantics node with label', (tester) async {
    await tester.pumpObers(
      const OiIcon(icon: testIcon, label: 'star icon'),
    );
    final semanticsWidgets =
        tester.widgetList<Semantics>(find.byType(Semantics));
    final matching =
        semanticsWidgets.where((s) => s.properties.label == 'star icon').toList();
  ...
  testWidgets('decorative icon renders Icon widget', (tester) async {
    await tester.pumpObers(
      const OiIcon.decorative(icon: testIcon),
    );
    expect(find.byType(Icon), findsOneWidget);
  });
  ...
  testWidgets('defaults to body font size when size is null', (tester) async {
    ...
    await tester.pumpObers(
      const OiIcon(icon: testIcon, label: 'default size'),
    );
    final iconWidget = tester.widget<Icon>(find.byType(Icon));
